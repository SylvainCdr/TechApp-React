...
import { ref, uploadBytes, getDownloadURL, deleteObject } from "firebase/storage"; // Importer deleteObject
import styles from "./style.module.scss";

export default function TechniciansPage() {
  const [technicians, setTechnicians] = useState([]);
  const [formData, setFormData] = useState({ firstName: "", lastName: "", role: "", phone: "", email: "", urlPhoto: "" });
  ...
  const fetchTechnicians = async () => {
    try {
      ...
      const techniciansList = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ firstName: "", lastName: "", role: "", phone: "", email: "", urlPhoto: "" });
      setPhoto(null);
      setEditId(null);
      fetchTechnicians();
    } catch (error) {
      console.error("Erreur lors de l'ajout ou de la mise à jour du technicien : ", error);
    }
  };

  // Fonction pour remplir le formulaire pour la modification
  const handleEdit = (technician) => {
    setFormData({ firstName: technician.firstName, lastName: technician.lastName, role: technician.role, phone: technician.phone, email: technician.email, urlPhoto: technician.urlPhoto });
  ...
  const handleDelete = async (technician) => {
    try {
      ...
      if (technician.urlPhoto) {
        // Extraire le nom du fichier et décoder les caractères encodés
        const fileName = decodeURIComponent(technician.urlPhoto.split('/').pop().split('?')[0]);
        const photoRef = ref(storage, `${fileName}`);
        await deleteObject(photoRef); // Utilisez deleteObject pour supprimer l'objet
      }
      ...
      <ul className={styles.techniciansList}>
        {technicians.map(technician => (
          <li key={technician.id} className={styles.technicianItem}>
            ...
            {technician.urlPhoto && <img src={technician.urlPhoto} alt={`${technician.firstName} ${technician.lastName}`} />}
            <button onClick={() => handleEdit(technician)}>Modifier</button>
            <button onClick={() => handleDelete(technician)}>Supprimer</button>
          </li>
        ))}
      </ul>
    </div>
