...
const generateReportPdf = async (report, technicians) => {
  ...
  for (let index = 0; index < report.actionsDone.length; index++) {
    ...
    const wrappedText = doc.splitTextToSize(actionText, 260); // Utilise toute la largeur pour la description
    doc.setFontSize(12);
    doc.text(wrappedText, 10, yPosition + 5); // Position de la description au-dessus des photos
    yPosition += 25; // Incrémente la position Y après la description
  ...
  if (hasRemarques) {
    ...
    for (let index = 0; index < report.remarques.length; index++) {
      ...
      const wrappedText = doc.splitTextToSize(remarqueText, 200); // Utilise toute la largeur pour la remarque
      doc.setFontSize(12);
